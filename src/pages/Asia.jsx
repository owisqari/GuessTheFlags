...
const Asia = () => {
  ...
  const getAllCountries = async () => {
    try {
      ...
      // Filter countries to only include those in Asia
      const asianCountries = countries.filter(
        (country) => country.region && country.region === "Asia"
      );
      const newArr = shuffleArray(asianCountries);
      return newArr;
    } catch (error) {
    ...
    <div className="container mt-5">
      {currentCountry && (
        <div className="text-center">
          ...
          <p className="text-success">Score: {score} </p>
